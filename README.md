...
This document details the engineering choices, performance constraints, and architectural layout patterns chosen to develop the **Gler Waitlist Admin Dashboard**.
...
Instead of anchoring page state inside localised client memory hooks (`useState`), this dashboard implements **URL Search Parameters as the Single Source of Truth**.
...
## 📊 3. Data Engineering Choices

### 🗄️ In-Memory Asynchronous Data Repository Layer

A critical choice was made to utilise a static, hard-copy data layer (`src/data/mock-providers.ts`) containing **55+ highly detailed, realistic UK service vendors** combined with an **asynchronous database simulation layer (`src/lib/db-repository.ts`)**.
...
Native browser date pickers (`<input type="date">`) hardcode presentation layouts based on device locale settings and reject custom string placeholder overrides. To mirror the exact Figma specifications, we engineered custom floating outline containers utilising **Shadcn’s Popover and Calendar** primitives.
...
4. **Bulk Selection Worker Pipelines:** For mass actions ("Select All 10,000 across pages"), moving checkbox collection out of component array structures and tracking selections via a centralised backend batch processing API route ensures memory usage stays at zero on the browser.
